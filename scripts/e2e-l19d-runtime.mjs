...
async function findFreePort(start = 3140) {
  ...
  fail("Could not find a free local port for L19D runtime E2E.");
}
...
try {
  ...
  await new Promise((resolve, reject) => {
    const child = spawn("node", ["scripts/e2e-v7-l19d-admin-information-architecture-runtime.mjs"], {
      cwd: process.cwd(),
      env: { ...process.env, BASE_URL: baseUrl },
      stdio: "inherit",
    });
    ...
    child.once("exit", (code, signal) =>
      code === 0
        ? resolve()
        : reject(new Error(`L19D runtime E2E failed: code=${code} signal=${signal ?? "none"}`)),
    );
  });
} finally {
  await cleanup();
