...
const required = [
  "V10_7_DELIVERY_MANIFEST.json",
  "V10_7_DELIVERY_NOTES.md",
  "architecture/phase-10.7/ReadyScore_V10_7_Access_Plans_Redesign.md",
  "app/access/page.tsx",
];
for (const file of required) {
  if (!fs.existsSync(path.join(root, file))) throw new Error(`MISSING: ${file}`);
  console.log(`PASS: required V10.7 artifact: ${file}`);
}

const manifest = JSON.parse(fs.readFileSync(path.join(root, "V10_7_DELIVERY_MANIFEST.json"), "utf8"));
const expected = {
  version:"V10.7", status:"IMPLEMENTATION_CANDIDATE", delivery:"FULL", scope:"ACCESS_PLANS_REDESIGN", baseline:"V10.6",
  ...
  historicalContentMutation:false, implementationMode:"PRESENTATION_ONLY", runtimeMode:"CUSTOMER_ACCESS_PLANS"
};
for (const [key,value] of Object.entries(expected)) if (manifest[key] !== value) throw new Error(`FAIL: manifest ${key}`);
console.log("PASS: manifest and V10.6 baseline");
...
"v10:7:gate":"node scripts/validate-v10-7-access-plans-redesign.mjs"
};
...
const page=fs.readFileSync(path.join(root,"app/access/page.tsx"),"utf8");
const source=page;
for (const marker of [
"Current access","Included capabilities","Assessment access","Upgrade / Get More","Purchase boundary",
"getCommercialCatalog","getActiveProductsForUser","listUserEntitlements","getUpgradeQuote",
"getReassessmentEligibility","getScalevCheckoutConfiguration","Get access","Upgrade",
"/api/scalev/checkout","Cross-Test Profile","Single Test","entitlement"
]) {
  if(!source.toLowerCase().includes(marker.toLowerCase())) throw new Error(`FAIL: implementation marker ${marker}`);
  console.log(`PASS: implementation marker: ${marker}`);
}

const notes=fs.readFileSync(path.join(root,"V10_7_DELIVERY_NOTES.md"),"utf8");
for(const marker of [
...
"app/reports/page.tsx","app/results/page.tsx","app/activity/page.tsx",
"app/result/[attemptId]/page.tsx","app/reassessment/[type]/page.tsx"
]){
...
if(fs.existsSync(migrationDir)){
  const suspicious=fs.readdirSync(migrationDir).filter(name=>/v10[._-]?7|access-plans/i.test(name));
  if(suspicious.length) throw new Error(`FAIL: V10.7 migration introduced: ${suspicious.join(", ")}`);
}
console.log("PASS: no V10.7 migration introduced");
console.log("V10.7 ACCESS & PLANS REDESIGN CONTRACT GATE: PASS");
