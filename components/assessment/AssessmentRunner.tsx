...
type AssessmentType="free"|"premium"|"riasec";
...
export default function AssessmentRunner({type}:{type:AssessmentType}){
  ...
  useEffect(()=>{
    const saved=window.sessionStorage.getItem(STORAGE_PREFIX+type);
    if(!saved)return;
    setResuming(true);
    fetch(`/api/assessment/${saved}`)
      ...
      .catch(()=>window.sessionStorage.removeItem(STORAGE_PREFIX+type))
  ...
  async function start(){
    ...
    try{
      const response=await fetch("/api/assessment/start",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({type})});
      const data=await response.json();
      ...
      window.sessionStorage.setItem(STORAGE_PREFIX+type,data.attemptId);
  ...
  async function abandon(){
    if(!attemptId)return;
    setBusy(true);
    try{await fetch(`/api/assessment/${attemptId}/abandon`,{method:"POST"});}finally{window.sessionStorage.removeItem(STORAGE_PREFIX+type);setAttemptId("");setQuestions([]);setAnswers({});setCurrent(0);setShowReview(false);setConfirmSubmit(false);setBusy(false);}
  }
    ...
    <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto max-w-5xl px-4 py-4 sm:px-6">
        <div className="flex items-center justify-between gap-4">
          <div className="min-w-0"><p className="text-xs font-bold uppercase tracking-wide text-indigo-600">ReadyScore {isRiasec?"RIASEC":isPremium?"Premium":"Free"}</p><p className="mt-1 truncate text-sm font-semibold text-slate-900">Assessment kesiapan Anda</p></div>
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_280px]">
        <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm sm:p-9">
          <div className="flex items-center justify-between gap-3"><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">{question.domain}</span><span className="text-xs font-semibold text-slate-400">{question.code}</span></div>
          <h1 className="mt-7 text-2xl font-bold leading-tight tracking-tight sm:text-3xl">{question.text}</h1>
          <p className="mt-3 text-sm leading-6 text-slate-500">Pilih jawaban yang paling menggambarkan kondisi Anda saat ini.</p>
...
function Intro({type,busy,message,onStart}:{type:AssessmentType;busy:boolean;message:string;onStart:()=>void}){
  ...
  const title=riasec?"Temukan profil minat Anda.":premium?"Assessment Premium yang lebih lengkap.":"Mulai dengan assessment gratis.";
  const description=riasec?"Ukur pola minat Anda melalui enam dimensi RIASEC dengan 60 pertanyaan terstruktur.":premium?"Ukur kesiapan Anda dengan cakupan yang lebih luas menggunakan Question Bank dan scoring model ReadyScore yang sama.":"Dapatkan gambaran awal kesiapan Anda melalui assessment singkat yang terstruktur.";
  const count=riasec?"60":premium?"100":"20";
  const button=riasec?"Mulai Assessment RIASEC":premium?"Mulai Assessment Premium":"Mulai Assessment Gratis";
  return <main className="min-h-screen bg-slate-50 px-4 py-10 text-slate-950 sm:px-6"><div className="mx-auto max-w-3xl"><div className="rounded-[32px] border border-slate-200 bg-white p-7 shadow-sm sm:p-10"><div className="inline-flex items-center gap-2 rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-700"><ShieldCheck className="h-4 w-4"/>ReadyScore {riasec?"RIASEC":premium?"Premium":"Free"}</div><h1 className="mt-5 text-4xl font-bold tracking-tight">{title}</h1><p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">{description}</p><div className="mt-8 grid gap-3 sm:grid-cols-3">{[[count,"soal"],[riasec?"6":"1","dimensi / Question Bank"],["5","pilihan jawaban"]].map(([value,label])=><div key={label} className="rounded-2xl bg-slate-50 p-4"><p className="text-2xl font-bold">{value}</p><p className="mt-1 text-xs text-slate-500">{label}</p></div>)}</div><div className="mt-8 rounded-2xl border border-slate-200 p-5"><p className="text-sm font-bold">Sebelum mulai</p><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-600"><li>• Tidak ada jawaban benar atau salah.</li><li>• Jawab berdasarkan kondisi Anda saat ini.</li><li>• Anda dapat kembali ke soal sebelumnya.</li><li>• Setelah submit, hasil dibuat sebagai snapshot assessment.</li></ul></div><button onClick={onStart} disabled={busy} className="mt-7 inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-6 py-3.5 text-sm font-bold text-white disabled:opacity-40">{busy?"Menyiapkan...":button}<ArrowRight className="h-4 w-4"/></button>{message&&<p className="mt-4 rounded-xl bg-amber-50 p-4 text-sm text-amber-800">{message}</p>}</div></div></main>;
}

function LoadingScreen({label}:{label:string}){return <main className="flex min-h-screen items-center justify-center bg-slate-50 px-6"><div className="rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm"><div className="mx-auto h-8 w-8 animate-spin rounded-full border-4 border-slate-200 border-t-indigo-600"/><p className="mt-4 text-sm font-semibold text-slate-600">{label}</p></div></main>;}
